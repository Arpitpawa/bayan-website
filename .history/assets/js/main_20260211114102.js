...
/* ======================================================
   BAYAN LUXURY BOOKING BAR – COMPLETE FINAL VERSION
====================================================== */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  guestSummary.addEventListener("click", () => {
    guestDropdown.style.display =
      guestDropdown.style.display === "flex" ? "none" : "flex";
  });

  /* Close when clicking outside */
  document.addEventListener("click", (e) => {
    if (!guestDropdown.contains(e.target) && !guestSummary.contains(e.target)) {
  ...
  function updateNightDisplay() {
    ...
    if (nights > 0) {
      nightCount.textContent = nights === 1 ? "1 Night" : `${nights} Nights`;

      nightCount.classList.add("highlight");
      setTimeout(() => nightCount.classList.remove("highlight"), 300);
    }
  }
  ...
  checkOut.addEventListener("change", updateNightDisplay);
...
function renderCalendar(date) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    // Random availability demo styling
    if (day % 7 === 0) div.classList.add("noarrival");
    else if (day % 5 === 0) div.classList.add("minstay");
