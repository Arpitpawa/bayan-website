...
function updateSlider() {
  ...
  =============================== */
  if (isMobile) {
    const slide = slides[1]; // 👈 pick best one (Kumbha Bagh)

    centerImage.src = slide.image;
    centerTitle.textContent = slide.title;
    centerText.textContent = slide.text;

    // Kill background layers
    bgLayers.forEach((bg) => {
      bg.classList.remove("active");
      bg.style.backgroundImage = "none";
    });

    return; // ⛔ STOP HERE — no slider logic
  }
